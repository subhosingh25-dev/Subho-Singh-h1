...
// Convert Base64 URL to Uint8Array for VAPID applicationServerKey
...
// Auto-subscribe device to Free Backend WebPush Server
async function subscribeUserToPush(assignedProduct: { name: string; code: string; url: string }) {
  ...
  try {
    ...
    if (!sub) {
      let vapidPublicKey = "BEl62iUYgUivxIkv69yViEuiBIa-Ib9-SkvMeAtA3LFgDzkrxZJjSgSnfckjBJuBkr3qBUYIHBQFLXYp5Nksh8U";
      try {
    ...
    if (sub) {
      ...
      console.log('[WebPush] Device registered on free push server successfully!');
    }
  } catch (err) {
    console.warn('[WebPush] Registration note:', err);
  }
}
...
export default function App() {
    ...
    if ('serviceWorker' in navigator) {
      navigator.serviceWorker.register('/sw.js', { scope: '/' })
        .then(async (reg) => {
          ...
          if (alreadyRevealed) {
            subscribeUserToPush(assignedProduct);
  ...
  // Initialize OneSignal for background OS-level Cloud Web Push if VITE_ONESIGNAL_APP_ID is configured
  useEffect(() => {
  ...
  const handleReveal = () => {
    ...
    // Register User with Self-Hosted Free WebPush Server
    subscribeUserToPush(assignedProduct);
